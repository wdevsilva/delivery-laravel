...
@endphp
            ...
            <form action="{{ url('/pedido/confirmar') }}" method="post" id="form-pedido" onsubmit="return validaPagamento()">
                ...
                <?php if ($config->config_aberto == 0) : ?>
                    ...
                    <h3 class="text-center">
                        <br>
                        <?php echo isset($config->config_horario) ? $config->config_horario : ''; ?>
                    </h3>
                <?php else : ?>
                    <?php
                    // IMPORTANTE: Total já vem calculado do controller COM desconto do cupom aplicado
                    $total_com_desconto = $total;

                    // Para exibição do desconto, usar subtotal original
                    ...
                    // TODO: Implementar fidelidade no Laravel
                    /*
                    ...
                    */
                    ...
                    if (count($itens) > 0) : ?>
                        <section id="pedido-itens">
                            ...
                            <?php
                            $total_itens = 0;
                            foreach ($itens as $cart) :
                                // Verifica se config_taxa_categorias não é nulo antes de fazer explode
                                $taxa_categorias = !empty($config->config_taxa_categorias) ? explode(',', $config->config_taxa_categorias) : [];
                            ...
                            ?>
                                <div class="row">
                                    ...
                                    <div class="col-md-2 col-xs-5">
                                        R$ <?= number_format($cart->item_preco * $cart->qtde, 2, ',', '.') ?>
                        ...
                        <section id="pedido-enderecos">
                            ...
                            <select class="form-control" name="pedido_local" id="pedido_local">
                                ...
                                <?php if ($config->config_retirada == 1) : ?>
                                    <option value="0" data-cep="0">Retirar no Local</option>
                                <?php endif;
                                if($config->config_entrega_pedido == 1){
                                ?>
                        ...
                        $fidelidade_ativo = isset($config->config_fidelidade_ativo) && $config->config_fidelidade_ativo == 1;

                        if ($fidelidade_ativo && isset($_SESSION['__CLIENTE__ID__'])) {
                            // TODO: Implementar busca de saldo de pontos no Laravel
                            /*
                            $fidelidadeModel = new fidelidadeModel();
                            ...
                            $pontos_minimo = isset($config->config_pontos_para_resgatar) ? (int)$config->config_pontos_para_resgatar : 100;
                            $valor_resgate = isset($config->config_valor_resgate_pontos) ? (float)$config->config_valor_resgate_pontos : 10.00;
                            $max_desconto = isset($config->config_fidelidade_max_desconto) ? (float)$config->config_fidelidade_max_desconto : 0.00;
                            $tipo_programa = isset($config->config_fidelidade_tipo) ? $config->config_fidelidade_tipo : 'pontos';
                            ...
                            */
                            $pontos_disponiveis = 0; // Temporariamente desabilitado
                            if (false) {
                        ?>
                        ...
                        <div class="form-group">
                            ...
                            <select disabled class="form-control" name="forma_pagamento" id="forma-pagamento" required>
                                ...
                                <?php if (($pagamentos[0]->pagamento_status ?? 0) == 1) : ?>
                        ...
                        <div class="divi-btn-finaliza">
                            <button type="submit" disabled class="btn btn-block btn-success text-uppercase" id="btn-pedido-concluir" <?php if ($total <= 0) : ?> disabled<?php endif; ?>>
    ...
    <script type="text/javascript">
        ...
        <?php if ($total < $config->config_pedmin) : ?>
    ...
    <script>
        ...
        const configTaxaCartao = <?php echo json_encode($config); ?>;

        // Taxa por categoria - disponibilizar para o JavaScript
        <?php if (isset($config->config_taxa_tipo) && $config->config_taxa_tipo === 'taxa_por_categoria'): ?>

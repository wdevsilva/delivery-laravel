<header>
    ...
    <div class="brand">
        <div class="logo" aria-hidden="true">
            <?php if ((new configModel)->get_config()->config_foto != '') {
                // Verificar se o arquivo existe
                $logoPath = __DIR__ . '/../../logo/' . $_SESSION['base_delivery'] . '/' . (new configModel)->get_config()->config_foto;
                $logoUrl = file_exists($logoPath)
                    ? $baseUri . '/logo/' . $_SESSION['base_delivery'] . '/' . (new configModel)->get_config()->config_foto
        ...
        <div class="store-info">
            <div class="store-name"><?= (isset($data['config']->config_nome)) ? $data['config']->config_nome : ''; ?></div>
            <div class="store-sub">
                <a href="http://maps.google.com/maps?daddr=<?= (new configModel)->get_config()->config_endereco; ?>&amp;ll=" target="__blank">
                    <i class="fa fa-map-marker" aria-hidden="true"></i> <?= (new configModel)->get_config()->config_endereco; ?>
                </a>
            </div>
            <div class="status-row">
                <div class="<?php echo ((new configModel)->get_config()->config_aberto == 1) ? 'badge-success' : 'badge-danger' ?>" aria-label=""><?php if ((new configModel)->get_config()->config_aberto == 1) { ?> Aberto <?php } else { ?> Fechado <?php } ?></div>
        ...
        <div class="modal-content" style="min-height: 0% !important;">
            ...
            <div class="modal-body">
                <table class="table" style="width: 200px; text-align: center; margin: 0 auto;">
                    <?php
                    $config_domingo = (new configModel)->get_config()->config_domingo;
                    ...
                    echo $domingo;
                    //-----------------------------------------------------------------------------------------------------------------------------
                    $config_segunda = (new configModel)->get_config()->config_segunda;
                    ...
                    echo $segunda;
                    //-----------------------------------------------------------------------------------------------------------------------------
                    $config_terca = (new configModel)->get_config()->config_terca;
                    ...
                    echo $terca;
                    //-----------------------------------------------------------------------------------------------------------------------------
                    $config_quarta = (new configModel)->get_config()->config_quarta;
                    ...
                    echo $quarta;
                    //-----------------------------------------------------------------------------------------------------------------------------
                    $config_quinta = (new configModel)->get_config()->config_quinta;
                    ...
                    echo $quinta;
                    //-----------------------------------------------------------------------------------------------------------------------------
                    $config_sexta = (new configModel)->get_config()->config_sexta;
                    ...
                    echo $sexta;
                    //-----------------------------------------------------------------------------------------------------------------------------
                    $config_sabado = (new configModel)->get_config()->config_sabado;

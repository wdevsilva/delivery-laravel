(function(f) {
    ...
    i.ext.buttons.print = {
        ...
        action: function(b, c, i, d) {
            ...
            cN = []; //className
            for(zz=0;zz<=c.header().columns().nodes().length -1; zz++){
                cN[zz] = c.header().columns().nodes()[zz][0].className;
            }
            var a = c.buttons.exportData(d.exportOptions),
                k = function(b, a, cn) {
                    for (var c = "<tr>", d = 0, e = b.length; d < e; d++) c += "<" + a + " class='"+cn[d]+"'>" + b[d] + "</" + a + ">";
                    return c + "</tr>"
                },
                b = '<table class="' + c.table().node().className + '">';
                //console.log( a.header )
                d.header && (b += "<thead>" + k(a.header, "th",cN) + "</thead>");
                //c.header().columns().nodes()[4].innerHTML
                //c.header().columns().nodes()[4].className

            for (var b = b + "<tbody>", l = 0, m = a.body.length; l < m; l++) b += k(a.body[l], "td",cN);
            b += "</tbody>";
            d.footer && a.footer && (b += "<tfoot>" + k(a.footer, "th",cN) + "</tfoot>");
            var g = e.open("", ""),
                a = d.title;

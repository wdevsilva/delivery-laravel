...
const modalBody = {
    payer:    $("#modal-body-payer"),// informações da empresa
    payment:  $('#modal-body-payment'),  // informações para a empresa realziar o pagamento
...
const sendFormDataToGeneratePixPayment = async (data) => {
    ...
    const response = await fetch(`pagamento/public/payment/create.php`, {
        method:    "POST",
...
const showApprovedPaymentStatus = ( externalReference ) => {
    const eventSource = new EventSource(`pagamento/public/payment/sse-status-payment.php?external_reference=${externalReference}`);
